...
def plot_predictions_as_histogram(predictions, truths, feature_type, dataset):
    ...
    plt.savefig(f'../graphics/plots/prediction-histogram-{feature_type}.png', dpi=300)
    plt.close()


def plot_predictions_as_density(predictions, feature_type):
    """
    ...
    plt.ylabel('count')
    plt.legend(loc='upper left')
    plt.savefig(f'../graphics/plots/prediction-density-{feature_type}.png', dpi=300)
    plt.close()


def plot_predictions_as_scatterplot(predictions, truths, feature_type):
    """
    ...
    plt.legend(loc='upper left')
    plt.savefig(f'../graphics/plots/prediction-scatter-{feature_type}.png', dpi=300)
    plt.close()
...
def plot_desired_scatterplot(truths, feature_type):
    ...
    plt.title(f'Desired predictions using {feature_type}')
    plt.xlabel('true quality')
...
def plot_predictions(feature_type, predictions=None, timeseries=False, dataset='split-10'):
    if predictions is None:
        predictions = os.listdir('../predictions/')
        predictions = sorted([m for m in predictions if m.startswith(f"{feature_type}-{'LSTM' if timeseries else '0'}")],
    ...
    plot_predictions_as_density(predictions, feature_type)
    plot_predictions_as_scatterplot(predictions, truths, feature_type)
